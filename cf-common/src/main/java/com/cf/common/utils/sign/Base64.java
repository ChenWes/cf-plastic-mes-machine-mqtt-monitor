package com.cf.common.utils.sign;
...
public final class Base64 {
    ...
    private static final char UPPER_Z_CHARACTER = 'Z';
    private static final char UPPER_A_CHARACTER = 'A';
    private static final char LOWER_Z_CHARACTER = 'z';
    private static final char LOWER_A_CHARACTER = 'a';
    private static final char CHARACTER_9 = '9';
    private static final char CHARACTER_0 = '0';
    private static final int UPPER_START_INDEX = 0;
    private static final int UPPER_END_INDEX = 25;
    private static final int LOWER_START_INDEX = 26;
    private static final int LOWER_END_INDEX = 51;
    private static final int NUMBER_START_INDEX = 52;
    private static final int NUMBER_END_INDEX = 61;

    static {
        for (int i = 0; i < BASE_LENGTH; ++i) {
            BASE_64_ALPHABET[i] = -1;
        }
        for (int i = UPPER_Z_CHARACTER; i >= UPPER_A_CHARACTER; i--) {
            BASE_64_ALPHABET[i] = (byte) (i - UPPER_A_CHARACTER);
        }
        for (int i = LOWER_Z_CHARACTER; i >= LOWER_A_CHARACTER; i--) {
            BASE_64_ALPHABET[i] = (byte) (i - LOWER_A_CHARACTER + 26);
        }

        for (int i = CHARACTER_9; i >= CHARACTER_0; i--) {
            BASE_64_ALPHABET[i] = (byte) (i - CHARACTER_0 + 52);
        }

        BASE_64_ALPHABET['+'] = 62;
        BASE_64_ALPHABET['/'] = 63;

        for (int i = UPPER_START_INDEX; i <= UPPER_END_INDEX; i++) {
            LOOK_UP_BASE_64_ALPHABET[i] = (char) ('A' + i);
        }

        for (int i = LOWER_START_INDEX, j = 0; i <= LOWER_END_INDEX; i++, j++) {
            LOOK_UP_BASE_64_ALPHABET[i] = (char) ('a' + j);
        }

        for (int i = NUMBER_START_INDEX, j = 0; i <= NUMBER_END_INDEX; i++, j++) {
    ...
    public static byte[] decode(String encoded) {
        ...
        if (!isData((d3)) || !isData((d4))) {
            if (isPad(d3) && isPad(d4)) {
                ...
                boolean lastFourBitsNotZero = (b2 & 0xf) != 0;
                if (lastFourBitsNotZero) {
                    return null;
                }
                ...
                boolean lastTwoBitsNotZero=(b3 & 0x3) != 0;
                if (lastTwoBitsNotZero) {
                    return null;
                }
